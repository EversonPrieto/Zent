...
export default function CreateTaskModal({
  ...
  // inline create label
  const [editingLabelId, setEditingLabelId] = useState<string | null>(null); // '__new__' or null
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm animate-in fade-in duration-200`}>
      <div
        className={`relative w-full max-w-lg max-h-[90vh] overflow-hidden rounded-2xl border shadow-2xl animate-in slide-in-from-bottom-4 duration-300 flex flex-col ${themeClasses.bg.primary} ${themeClasses.border.primary}`}
      >
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                <div className={`inline-flex items-center gap-1.5 rounded-full ${currentStatusConfig.bg} px-2.5 py-1`}>
                  ...
                  <span className={`text-xs font-medium ${currentStatusConfig.color}`}>{currentStatusConfig.label}</span>
                </div>
                <Sparkles className="h-4 w-4 text-violet-400" />
              </div>
              <h2 className={`text-2xl font-bold ${themeClasses.text.primary}`}>Nova task</h2>
              <p className={`mt-1 text-sm ${themeClasses.text.tertiary}`}>Crie uma nova task para organizar seu trabalho</p>
            </div>
        ...
        <div className="overflow-y-auto flex-1">
          <div className="p-6 space-y-5">
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-2 px-4 py-3 text-sm transition-all first:rounded-t-lg last:rounded-b-lg ${
                            isSelected ? `${config.bg} ${config.color}` : `${themeClasses.text.secondary} hover:${themeClasses.bg.hover}`
                          }`}
                        >
                          <Icon className="h-4 w-4" />
                          <span>{config.label}</span>
                          {isSelected && <div className="ml-auto h-2 w-2 rounded-full bg-emerald-500" />}
                        </button>
                      );
                    })}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                {priorityOptions.map((option) => {
                  ...
                  return (
                    ...
                    >
                      <config.icon
                        className={`h-5 w-5 ${config.color} ${isSelected ? 'scale-110' : ''} transition-transform`}
                      />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  disabled={projectCompleted}
                  className={`w-full rounded-xl border ${themeClasses.border.primary} ${themeClasses.bg.subtle} px-4 py-2.5 ${themeClasses.text.primary} outline-none transition-all focus:border-violet-500 focus:ring-1 focus:ring-violet-500 disabled:opacity-50`}
                />
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <button
                    ...
                    className={`w-full flex items-center justify-between rounded-xl border ${themeClasses.border.primary} ${themeClasses.bg.subtle} px-4 py-2.5 ${themeClasses.text.primary} outline-none transition-all hover:${themeClasses.bg.hover} focus:border-violet-500 focus:ring-1 focus:ring-violet-500 ${projectCompleted ? 'opacity-50 cursor-not-allowed' : ''}`}
                    disabled={projectCompleted}
                  >
                    <span className="text-sm">
                      {selectedAssignees.length > 0
                        ? `${selectedAssignees.length} responsável(is)`
            ...
            <div>
              <label className={`mb-2 flex items-center gap-2 text-sm font-medium ${themeClasses.text.secondary}`}>
                <span className="inline-flex items-center justify-center h-6 w-6 rounded-lg bg-violet-500/10 border border-violet-500/20">
              ...
              {labelsLoading ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className={`text-xs ${themeClasses.text.muted}`}>Selecionar via checkbox</span>
                  ...
                  {editingLabelId === '__new__' && !projectCompleted && (
                    <div className={`rounded-xl border ${themeClasses.border.primary} ${themeClasses.bg.subtle} p-3`}>
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          value={editingLabelName}
                          onChange={(e) => setEditingLabelName(e.target.value)}
                          placeholder="Nome da label"
                          className={`rounded-lg border ${themeClasses.border.primary} ${themeClasses.bg.primary} px-3 py-2 text-sm outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500`}
                        />
                        <input
                          type="color"
                      ...
                      <div className="mt-2 flex gap-2">
                        ...
                        <button
                          ...
                          className={`flex-1 rounded-lg px-3 py-2 text-sm font-medium ${themeClasses.text.tertiary} hover:${themeClasses.bg.hover} hover:${themeClasses.text.primary}`}
                        >
                          Cancelar
                        </button>
                      </div>
